...
`<id>-<slug>`: id гарантирует уникальность и переживает любые правки названия. Слаги стран и типов
проверяются против списка зарезервированных (`all`, `cart`, `info`, `contacts`, `purchases`,
`unsubscribe`, `api`, `admin`, `static`, `media`, `sitemap.xml`, `robots.txt`).
