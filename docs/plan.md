...
**M5 — подчистка и документация.** Обновление `CLAUDE.md` и `CONTEXT.md` под фактическое
состояние, вычитка ADR, правки дизайна, накопленные с M2 (заказчик собирает их отдельным
списком). Валютный код, `djmoney` и его цели `update-rates`/`expire` удалены раньше — на M1 и
M2b; Verdoc-компоненты фронта — на M3. (vue-router и vue-i18n остаются — ADR-0010.)
